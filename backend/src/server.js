...
const q = async (sql, ...args) => await db.prepare(sql).all(...args);
...
async function audit(user, action, entity, entity_id, detail = '') {
  try {
    await db.prepare('INSERT INTO audit_logs (user_id, action, entity, entity_id, detail) VALUES (?, ?, ?, ?, ?)')
...
async function getStock(warehouse_id, product_id) {
  return await one('SELECT * FROM inventory WHERE warehouse_id = ? AND product_id = ?', warehouse_id, product_id);
}

// Low level movement helper (Must be called inside a transaction context)
async function recordMovement({ warehouse_id, product_id, type, qty, ref, note, user }) {
  let inv = await getStock(warehouse_id, product_id);
  if (!inv) {
    await db.prepare('INSERT INTO inventory (warehouse_id, product_id, stock_on_hand) VALUES (?, ?, 0)')
      .run(warehouse_id, product_id);
    inv = await getStock(warehouse_id, product_id);
  }
  ...
  await db.prepare('UPDATE inventory SET stock_on_hand = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
    .run(after, inv.id);
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    await audit(u, 'LOGIN', 'USER', u.id, 'User logged in successfully');
...
app.get('/api/managers', auth, async (req, res) => {
  ...
  if (targetWh) {
    res.json(await q("SELECT id, name, email, role, warehouse_id FROM users WHERE role IN ('MANAGER', 'SUPER_ADMIN') AND status = 'ACTIVE' AND (warehouse_id = ? OR warehouse_id IS NULL)", targetWh));
  } else {
    res.json(await q("SELECT id, name, email, role, warehouse_id FROM users WHERE role IN ('MANAGER', 'SUPER_ADMIN') AND status = 'ACTIVE'"));
  }
});
...
app.get('/api/dashboard', auth, async (req, res) => {
  ...
  const totalRow = await one(`SELECT COALESCE(SUM(stock_on_hand), 0) total FROM inventory i ${where}`, ...args);
  const total = totalRow ? totalRow.total : 0;
  const skusRow = await one(`SELECT COUNT(*) c FROM inventory i ${where}`, ...args);
  const skus = skusRow ? skusRow.c : 0;

  const rawAlerts = await q(`
  ...
  const alerts = rawAlerts.map(x => ({
    ...x,
    ads30: Number(x.ads30 || 0),
    stock_on_hand: Number(x.stock_on_hand || 0),
    doi30: Number(x.ads30 || 0) > 0 ? +(Number(x.stock_on_hand || 0) / Number(x.ads30 || 0)).toFixed(1) : null
  }));
  ...
  const inboundRow = await one(`SELECT COALESCE(SUM(quantity), 0) n FROM stock_movements ${inboundWhere}`, ...(targetWh ? [targetWh] : []));
  const inbound = inboundRow ? inboundRow.n : 0;
  const outboundRow = await one(`SELECT COALESCE(SUM(ABS(quantity)), 0) n FROM stock_movements ${outboundWhere}`, ...(targetWh ? [targetWh] : []));
  const outbound = outboundRow ? outboundRow.n : 0;

  res.json({ total_stock: total, sku_count: skus, inbound_today: inbound, outbound_today: outbound, alerts });
});

app.get('/api/analytics/doi', auth, async (req, res) => {
  const targetWh = getWarehouseFilter(req);
  const rows = targetWh ? await q('SELECT * FROM inventory WHERE warehouse_id = ?', targetWh) : await q('SELECT * FROM inventory');
  let stats = { critical: 0, healthy: 0, slow: 0, overstock: 0, no_sales: 0 };

  for (const i of rows) {
    const adsRow = await one(
    ...
    const ads = Number(adsRow ? adsRow.a : 0);

    const doi = ads > 0 ? Number(i.stock_on_hand) / ads : null;
...
app.post('/api/warehouses', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const { name, address, city, status } = req.body;
  if (!name) return res.status(400).json({ message: 'Warehouse name is required.' });

  const result = await db.prepare('INSERT INTO warehouses (name, address, city, status) VALUES (?, ?, ?, ?)')
    .run(name, address || '', city || '', status || 'ACTIVE');

  await audit(req.user, 'CREATE_WAREHOUSE', 'WAREHOUSE', result.lastInsertRowid, `Created warehouse ${name}`);
  res.json({ id: result.lastInsertRowid, message: 'Warehouse created successfully.' });
});

app.put('/api/warehouses/:id', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const { id } = req.params;
  const { name, address, city, status } = req.body;

  const w = await one('SELECT * FROM warehouses WHERE id = ?', id);
  if (!w) return res.status(404).json({ message: 'Warehouse not found.' });

  await db.prepare('UPDATE warehouses SET name = ?, address = ?, city = ?, status = ? WHERE id = ?')
    .run(name || w.name, address ?? w.address, city ?? w.city, status || w.status, id);

  await audit(req.user, 'UPDATE_WAREHOUSE', 'WAREHOUSE', id, `Updated warehouse ${name || w.name}`);
  res.json({ message: 'Warehouse updated successfully.' });
});

app.delete('/api/warehouses/:id', auth, allow('SUPER_ADMIN'), async (req, res) => {
  const { id } = req.params;
  const inv = await one('SELECT COUNT(*) c FROM inventory WHERE warehouse_id = ? AND stock_on_hand > 0', id);
  if (inv && inv.c > 0) {
    return res.status(400).json({ message: 'Cannot delete warehouse with active stock. Deactivate it instead.' });
  }

  try {
    await db.prepare('DELETE FROM warehouses WHERE id = ?').run(id);
    await audit(req.user, 'DELETE_WAREHOUSE', 'WAREHOUSE', id, `Deleted warehouse ID ${id}`);
...
app.get('/api/products', auth, async (req, res) => {
  res.json(await q('SELECT * FROM products ORDER BY product_name'));
});

app.post('/api/products', auth, allow('SUPER_ADMIN', 'MANAGER', 'WAREHOUSE_ADMIN'), async (req, res) => {
  const b = req.body;
  ...
  try {
    const existing = b.id
      ? await db.prepare('SELECT * FROM products WHERE id = ?').get(b.id)
      : await db.prepare('SELECT * FROM products WHERE sku = ?').get(b.sku);
    ...
    if (existing) {
      ...
      await audit(req.user, 'UPDATE', 'PRODUCT', existing.id, `Updated product SKU: ${b.sku} (${b.variant || 'No variant'})`);
    ...
    await audit(req.user, 'CREATE', 'PRODUCT', result.lastInsertRowid, `Created product SKU: ${b.sku} (${b.variant || 'No variant'})`);
  ...
  return async (req, res) => {
    ...
    try {
      const transaction = db.transaction(async () => {
        ...
        `).run(typeCode, num, b.warehouse_id, b.destination_warehouse_id || null, b.partner || '', req.user.id, assignedManagerId);

        for (const item of b.items) {
          await db.prepare('INSERT INTO document_items (document_id, product_id, quantity, price) VALUES (?, ?, ?, ?)')
            .run(doc.lastInsertRowid, item.product_id, item.quantity, item.price || 0);
        }

        await audit(req.user, 'SUBMIT_REQUEST', typeCode, doc.lastInsertRowid, `Submitted ${typeCode} request ${num} to Manager ID ${assignedManagerId || 'Global'}`);
        return doc.lastInsertRowid;
      });

      const docId = await transaction();
...
app.post('/api/document-items/:id/toggle-reject', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const item = await one('SELECT * FROM document_items WHERE id = ?', req.params.id);
  if (!item) return res.status(404).json({ message: 'Document item not found.' });

  const doc = await one('SELECT * FROM documents WHERE id = ?', item.document_id);
  ...
  await db.prepare('UPDATE document_items SET is_rejected = ? WHERE id = ?').run(newRejectedState, item.id);

  await audit(req.user, newRejectedState ? 'REJECT_ITEM' : 'RESTORE_ITEM', 'DOCUMENT_ITEM', item.id, `${newRejectedState ? 'Rejected' : 'Restored'} item ${item.product_id} in document ${doc.doc_number}`);
  res.json({ id: item.id, is_rejected: newRejectedState, message: newRejectedState ? 'Item ditolak (Rejected)' : 'Item diterima (Approved)' });
});

app.post('/api/documents/:id/approve', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const d = await one('SELECT * FROM documents WHERE id = ?', req.params.id);
  ...
  const items = await q('SELECT * FROM document_items WHERE document_id = ? AND (is_rejected IS NULL OR is_rejected = 0)', d.id);

  const processApproval = db.transaction(async () => {
    for (const it of items) {
      if (d.doc_type === 'INBOUND' || d.doc_type.startsWith('PO')) {
        await recordMovement({ warehouse_id: d.warehouse_id, product_id: it.product_id, type: 'INBOUND', qty: it.quantity, ref: d.doc_number, note: `Approved Purchase Order (PO) by Manager (${req.user.name})`, user: req.user });
      } else if (d.doc_type === 'OUTBOUND') {
        await recordMovement({ warehouse_id: d.warehouse_id, product_id: it.product_id, type: 'OUTBOUND', qty: -it.quantity, ref: d.doc_number, note: `Approved Outbound by Manager (${req.user.name})`, user: req.user });
      } else if (d.doc_type === 'TRANSFER') {
        await recordMovement({ warehouse_id: d.warehouse_id, product_id: it.product_id, type: 'TRANSFER_OUT', qty: -it.quantity, ref: d.doc_number, note: `Transfer Approved - In Transit by Manager (${req.user.name})`, user: req.user });
      }
    }

    const nextStatus = d.doc_type === 'TRANSFER' ? 'IN_TRANSIT' : 'APPROVED';
    await db.prepare('UPDATE documents SET status = ?, approved_by = ? WHERE id = ?').run(nextStatus, req.user.id, d.id);
    await audit(req.user, 'APPROVE', d.doc_type, d.id, `Manager ${req.user.name} approved document ${d.doc_number} (New Status: ${nextStatus})`);
  });
...
app.post('/api/documents/:id/reject', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const d = await one('SELECT * FROM documents WHERE id = ?', req.params.id);
  if (!d) return res.status(404).json({ message: 'Document not found.' });
  ...
  await db.prepare("UPDATE documents SET status = 'REJECTED', approved_by = ?, rejection_reason = ? WHERE id = ?")
    .run(req.user.id, reason || 'Rejected by Manager', d.id);

  await audit(req.user, 'REJECT', d.doc_type, d.id, `Manager ${req.user.name} rejected document ${d.doc_number}. Reason: ${reason || 'None'}`);
  res.json({ message: `Document ${d.doc_number} rejected.` });
});

// Receiving Inter-Warehouse Transfer Endpoint
app.post('/api/transfers/:id/receive', auth, allow('SUPER_ADMIN', 'MANAGER', 'WAREHOUSE_ADMIN'), async (req, res) => {
  const d = await one("SELECT * FROM documents WHERE id = ? AND doc_type = 'TRANSFER'", req.params.id);
  ...
  const items = await q('SELECT * FROM document_items WHERE document_id = ?', d.id);

  const processReceive = db.transaction(async () => {
    for (const it of items) {
      await recordMovement({ warehouse_id: d.destination_warehouse_id, product_id: it.product_id, type: 'TRANSFER_IN', qty: it.quantity, ref: d.doc_number, note: 'Transfer Received', user: req.user });
    }

    await db.prepare("UPDATE documents SET status = 'RECEIVED' WHERE id = ?").run(d.id);
    await audit(req.user, 'RECEIVE', 'TRANSFER', d.id, `Received transfer ${d.doc_number} at destination warehouse ID ${d.destination_warehouse_id}`);
  });
...
app.get('/api/stock-opname', auth, async (req, res) => {
  const targetWh = getWarehouseFilter(req);
...
app.post('/api/stock-opname', auth, allow('SUPER_ADMIN', 'MANAGER', 'WAREHOUSE_ADMIN', 'OPERATOR'), async (req, res) => {
  ...
  const inv = await getStock(warehouse_id, product_id);
  ...
  await audit(req.user, 'CREATE', 'STOCK_OPNAME', result.lastInsertRowid, `Recorded opname variance: ${variance}`);
  res.json({ id: result.lastInsertRowid, variance, status: 'PENDING_APPROVAL' });
});

app.post('/api/stock-opname/:id/adjust', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const op = await one('SELECT * FROM stock_opname WHERE id = ?', req.params.id);
  ...
  const processAdjustment = db.transaction(async () => {
    if (op.variance !== 0) {
    ...
    await db.prepare("UPDATE stock_opname SET status = 'APPROVED', approved_by = ? WHERE id = ?")
      .run(req.user.id, op.id);

    await audit(req.user, 'APPROVE', 'STOCK_OPNAME', op.id, `Approved stock opname adjustment variance: ${op.variance}`);
  });
...
app.get('/api/audit-logs', auth, allow('SUPER_ADMIN', 'MANAGER', 'WAREHOUSE_ADMIN'), async (req, res) => {
  const targetWh = getWarehouseFilter(req);
...
app.get('/api/users', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  res.json(await q('SELECT id, name, email, role, warehouse_id, status, created_at FROM users ORDER BY id'));
});

app.post('/api/users', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  ...
  try {
    ...
    await audit(req.user, 'CREATE', 'USER', result.lastInsertRowid, `Created user ${email} (${role})`);
...
app.put('/api/users/:id', auth, async (req, res) => {
  ...
  const existing = await one('SELECT * FROM users WHERE id = ?', targetId);
  ...
  try {
    ...
    await audit(req.user, 'UPDATE', 'USER', targetId, `Updated user ${newEmail}${password ? ' (Password changed)' : ''}`);
...
app.delete('/api/users/:id', auth, allow('SUPER_ADMIN', 'MANAGER'), async (req, res) => {
  const targetId = Number(req.params.id);
  ...
  try {
    ...
    await audit(req.user, 'DELETE', 'USER', targetId, `Deleted user ${existing.email}`);
...
// CSV Export Report
app.get('/api/reports/inventory.csv', auth, async (req, res) => {
  const targetWh = getWarehouseFilter(req);
  const where = targetWh ? 'WHERE i.warehouse_id = ?' : '';
  const args = targetWh ? [targetWh] : [];
  const rows = await q(`
